...
def emit_vh(path, sets):
    ...
    # NOTE: no `timescale in the output — the .vh is included INSIDE
    # src_interp (module scope: Gowin Verilog-2001 forbids root-scope
    # declarations, EX3209), which already sets the timescale.
    with open(path, 'w') as f:
        f.write("// GENERATED by tools/design_interp.py — do not hand-edit.\n")
        # Guard: the file must be includable AND listable. Gowin compiles
        # every project file (even include/*.vh) as a standalone unit, and
        # Verilog-2001 forbids root-scope declarations (EX3209) — so the
        # function body only exists when the includer asks for it. The file
        # alone compiles to nothing (comments + directives only).
        f.write("`ifdef SRC_INTERP_ROM_BODY\n")
